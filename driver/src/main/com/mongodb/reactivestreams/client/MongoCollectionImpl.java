...
class MongoCollectionImpl<T> implements MongoCollection<T> {
    ...
    @Override
    public MongoPublisher<Long> count() {
        return count(new BsonDocument(), new CountOptions());
    }

    @Override
    public MongoPublisher<Long> count(final Object filter) {
        return count(filter, new CountOptions());
    }

    @Override
    public MongoPublisher<Long> count(final Object filter, final CountOptions options) {
    ...
    @Override
    public MongoPublisher<Object> distinct(final String fieldName, final Object filter) {
        return distinct(fieldName, filter, new DistinctOptions());
    }

    @Override
    public MongoPublisher<Object> distinct(final String fieldName, final Object filter, final DistinctOptions options) {
        DistinctOperation operation = new DistinctOperation(namespace, fieldName)
                                      .filter(asBson(filter))
                                      .maxTime(options.getMaxTime(MILLISECONDS), MILLISECONDS);

        return Publishers.flatten(Publishers.publish(operation, this.options.getReadPreference(), executor)
                                            .map(new Function<BsonArray, List<Object>>() {
                                                @Override
    ...
    @Override
    public MongoPublisher<Document> aggregate(final List<?> pipeline) {
        return aggregate(pipeline, new AggregateOptions(), Document.class);
    }

    @Override
    public <C> MongoPublisher<C> aggregate(final List<?> pipeline, final Class<C> clazz) {
        return aggregate(pipeline, new AggregateOptions(), clazz);
    }

    @Override
    public MongoPublisher<Document> aggregate(final List<?> pipeline, final AggregateOptions options) {
        return aggregate(pipeline, options, Document.class);
    }

    @Override
    public <C> MongoPublisher<C> aggregate(final List<?> pipeline, final AggregateOptions options, final Class<C> clazz) {
    ...
    @Override
    public MongoPublisher<Document> mapReduce(final String mapFunction, final String reduceFunction) {
        return mapReduce(mapFunction, reduceFunction, new MapReduceOptions());
    }

    @Override
    public MongoPublisher<Document> mapReduce(final String mapFunction, final String reduceFunction, final MapReduceOptions options) {
        return mapReduce(mapFunction, reduceFunction, options, Document.class);
    }

    @Override
    public <C> MongoPublisher<C> mapReduce(final String mapFunction, final String reduceFunction, final Class<C> clazz) {
        return mapReduce(mapFunction, reduceFunction, new MapReduceOptions(), clazz);
    }

    @Override
    public <C> MongoPublisher<C> mapReduce(final String mapFunction, final String reduceFunction, final MapReduceOptions options,
                                           final Class<C> clazz) {
        if (options.isInline()) {
    ...
    @Override
    public MongoPublisher<BulkWriteResult> bulkWrite(final List<? extends WriteModel<? extends T>> requests) {
    ...
    @Override
    public MongoPublisher<Void> insertOne(final T document) {
        if (getCodec() instanceof CollectibleCodec) {
        ...
        return Publishers.publish(new InsertOperation(namespace, true, options.getWriteConcern(), requests), executor)
                         .map(new Function<WriteConcernResult, Void>() {
                             @Override
    ...
    @Override
    public MongoPublisher<Void> insertMany(final List<? extends T> documents) {
        return insertMany(documents, new InsertManyOptions());
    }

    @Override
    public MongoPublisher<Void> insertMany(final List<? extends T> documents, final InsertManyOptions options) {
        ...
        return Publishers.publish(new InsertOperation(namespace, options.isOrdered(), this.options.getWriteConcern(),
                requests), executor)
                .map(new Function<WriteConcernResult, Void>() {
                            @Override
                            public Void apply(final WriteConcernResult writeConcernResult) {
    ...
    @Override
    public MongoPublisher<DeleteResult> deleteOne(final Object filter) {
        return delete(filter, false);
    }

    @Override
    public MongoPublisher<DeleteResult> deleteMany(final Object filter) {
        return delete(filter, true);
    }

    @Override
    public MongoPublisher<UpdateResult> replaceOne(final Object filter, final T replacement) {
        return replaceOne(filter, replacement, new UpdateOptions());
    }

    @Override
    public MongoPublisher<UpdateResult> replaceOne(final Object filter, final T replacement, final UpdateOptions updateOptions) {
    ...
    @Override
    public MongoPublisher<UpdateResult> updateOne(final Object filter, final Object update) {
        return updateOne(filter, update, new UpdateOptions());
    }

    @Override
    public MongoPublisher<UpdateResult> updateOne(final Object filter, final Object update, final UpdateOptions options) {
        return update(filter, update, options, false);
    }

    @Override
    public MongoPublisher<UpdateResult> updateMany(final Object filter, final Object update) {
        return updateMany(filter, update, new UpdateOptions());
    }

    @Override
    public MongoPublisher<UpdateResult> updateMany(final Object filter, final Object update, final UpdateOptions options) {
        return update(filter, update, options, true);
    }

    @Override
    public MongoPublisher<T> findOneAndDelete(final Object filter) {
        return findOneAndDelete(filter, new FindOneAndDeleteOptions());
    }

    @Override
    public MongoPublisher<T> findOneAndDelete(final Object filter, final FindOneAndDeleteOptions options) {
    ...
    @Override
    public MongoPublisher<T> findOneAndReplace(final Object filter, final T replacement) {
        return findOneAndReplace(filter, replacement, new FindOneAndReplaceOptions());
    }

    @Override
    public MongoPublisher<T> findOneAndReplace(final Object filter, final T replacement, final FindOneAndReplaceOptions options) {
    ...
    @Override
    public MongoPublisher<T> findOneAndUpdate(final Object filter, final Object update) {
        return findOneAndUpdate(filter, update, new FindOneAndUpdateOptions());
    }

    @Override
    public MongoPublisher<T> findOneAndUpdate(final Object filter, final Object update, final FindOneAndUpdateOptions options) {
    ...
    @Override
    public MongoPublisher<Void> dropCollection() {
        return Publishers.publish(new DropCollectionOperation(namespace), executor);
    }

    @Override
    public MongoPublisher<Void> createIndex(final Object key) {
        return createIndex(key, new CreateIndexOptions());
    }

    @Override
    public MongoPublisher<Void> createIndex(final Object key, final CreateIndexOptions options) {
    ...
    @Override
    public MongoPublisher<Document> getIndexes() {
        return getIndexes(Document.class);
    }

    @Override
    public <C> MongoPublisher<C> getIndexes(final Class<C> clazz) {
        return Publishers.flatten(new ListIndexesOperation<C>(namespace, getCodec(clazz)), options.getReadPreference(), executor);
    }

    @Override
    public MongoPublisher<Void> dropIndex(final String indexName) {
        return Publishers.publish(new DropIndexOperation(namespace, indexName), executor);
    }

    @Override
    public MongoPublisher<Void> dropIndexes() {
        return dropIndex("*");
    }

    @Override
    public MongoPublisher<Void> renameCollection(final MongoNamespace newCollectionNamespace) {
        return renameCollection(newCollectionNamespace, new RenameCollectionOptions());
    }

    @Override
    public MongoPublisher<Void> renameCollection(final MongoNamespace newCollectionNamespace, final RenameCollectionOptions options) {
    ...
    private MongoPublisher<DeleteResult> delete(final Object filter, final boolean multi) {
        return Publishers.publish(new DeleteOperation(namespace, true, options.getWriteConcern(),
    ...
    private MongoPublisher<UpdateResult> update(final Object filter, final Object update, final UpdateOptions updateOptions,
                                                final boolean multi) {
    ...
    private MongoPublisher<UpdateResult> createUpdateResult(final MongoPublisher<BulkWriteResult> publisher) {
        return publisher.map(new Function<BulkWriteResult, UpdateResult>() {
            @Override
